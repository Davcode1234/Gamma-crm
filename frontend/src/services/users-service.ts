...
export async function getUserClientsChart(
  month: number,
  year: number,
  yearlySummary: boolean,
  userId: string
  ...
  const fetchUrl = yearlySummary
    ? `/api/dashboard/reckoning/user-clients-per-hour-yearly/${year}/${userId}`
    : `/api/dashboard/reckoning/user-clients-per-hour/${month}/${year}/${userId}`;
  try {
    const response = await fetch(`${import.meta.env.VITE_API_URL}${fetchUrl}`, {
      method: 'GET',
      credentials: 'include',
